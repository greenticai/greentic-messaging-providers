...
stage_core_component() {
  local name="$1"
  local target_src="${ROOT_DIR}/target/components/${name}.wasm"
...
stage_templates_component() {
  local staged=0
  copy_if_present \
    "${ROOT_DIR}/target/components/templates.wasm" \
    "${ROOT_DIR}/components/templates/templates.wasm" && staged=1 || true
  if [ "${staged}" -eq 0 ]; then
    copy_if_present \
      "${ROOT_DIR}/target/components/ai.greentic.component-templates.wasm" \
