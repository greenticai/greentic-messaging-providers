...
fn pack_has_extension_and_schema() -> Result<()> {
    ...
    assert!(
        workspace_root()
            .join("schemas/messaging/dummy/public.config.schema.json")
            .exists(),
...
fn invoke_send_smoke_test() -> Result<()> {
    ...
    let second_json = invoke_run(&engine, &component, &linker, canonical_cbor_bytes(&input))?;
    assert_eq!(first_json.get("ok"), Some(&Value::Bool(true)));
