...
mod bindings {
    wasmtime::component::bindgen!({
        path: "../../components/messaging-provider-webex/wit/messaging-provider-webex",
        world: "messaging-provider-webex",
    });
}

const TOKEN_KEY: &str = "WEBEX_BOT_TOKEN";
...
fn candidate_artifacts() -> Vec<PathBuf> {
    ...
    vec![
        root.join("target/components/messaging-provider-webex.wasm"),
        root.join("target/wasm32-wasip2/release/messaging_provider_webex.wasm"),
        root.join("target/wasm32-wasip2/wasm32-wasip2/release/messaging_provider_webex.wasm"),
        root.join("components/messaging-provider-webex/target/wasm32-wasip2/release/messaging_provider_webex.wasm"),
        root.join("packs/messaging-webex/components/messaging-provider-webex.wasm"),
    ]
}
...
fn ensure_component_artifact() -> Result<PathBuf> {
    ...
    let status = Command::new("cargo")
        ...
        .status()
        .context("running cargo component build for messaging-provider-webex")?;
    if !status.success() {
    ...
    for path in candidate_artifacts() {
        if path.exists() {
            let target = workspace_root().join("target/components/messaging-provider-webex.wasm");
            if !target.exists() {
...
impl bindings::greentic::http::http_client::Host for HostState {
    ...
    > {
        ...
        Ok(bindings::greentic::http::http_client::Response {
            status: 200,
            headers: vec![],
            body: Some(serde_json::to_vec(&json!({"id":"message-1"})).expect("resp bytes")),
        })
    }
}
...
#[test]
fn builds_webex_component() -> Result<()> {
...
fn pack_has_extension_and_schema() -> Result<()> {
    let pack_dir = workspace_root().join("packs/messaging-webex");
    ...
    let schema_ref = provider
        ...
        .and_then(|v| v.as_str())
        .unwrap_or_default();
    assert_eq!(schema_ref, "schemas/messaging/webex/config.schema.json");
    assert!(
    ...
    assert!(
        workspace_root()
            .join("schemas/messaging/webex/config.schema.json")
            .exists(),
...
fn invoke_send_smoke_test() -> Result<()> {
    ...
    assert_eq!(manifest.provider_type, "messaging.webex.bot");
    ...
    assert_eq!(
        ...
        Some(&Value::String("messaging.webex.bot".into()))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(&Value::String("webex:message-1".into()))
    );
    ...
    assert!(last_req.url.ends_with("/messages"));
    assert!(
        last_req
            .headers
            .iter()
            .any(|(k, v)| k == "Authorization" && v == "Bearer super-secret")
    );
    let body_json: Value = serde_json::from_slice(last_req.body.as_ref().expect("body set"))?;
    assert_eq!(
        body_json.get("roomId"),
        Some(&Value::String("room-1".into()))
    );
    assert_eq!(
        body_json.get("text"),
        Some(&Value::String("hello webex".into()))
    );
...
fn invoke_reply_smoke_test() -> Result<()> {
    ...
    assert!(
        resp_json
            ...
            .unwrap_or("")
            .starts_with("webex:"),
        "provider_message_id should start with webex:"
    );
