...
fn pack_has_extension_and_schema() -> Result<()> {
    ...
    assert!(
        workspace_root()
            .join("schemas/messaging/telegram/public.config.schema.json")
            .exists(),
    ...
    assert_eq!(
        ingress_ext
            ...
            .and_then(|v| v.as_str()),
        Some("messaging-ingress-telegram")
    );
    Ok(())
}
...
fn invoke_send_smoke_test() -> Result<()> {
    ...
    input.as_object_mut().expect("envelope object").insert(
        "config".to_string(),
        json!({ "api_base_url": "https://api.telegram.org" }),
    );
...
fn invoke_reply_smoke_test() -> Result<()> {
    ...
    envelope_obj.insert("reply_to_id".to_string(), json!("42"));
    envelope_obj.insert(
        "config".to_string(),
        json!({
            "default_chat_id": "123",
...
fn reply_requires_reply_to_id() -> Result<()> {
    ...
    input
        ...
        .insert("config".to_string(), json!({ "default_chat_id": "123" }));
    let (resp,) = invoke
        .call(
            &mut store,
